...
if __name__ == '__main__':
    ...
    # （Merge files: From the specified data file directory and PNG image directory,
    #       merge each data file with the corresponding PNG image file into a new PNG file.）

    # 恢复文件: 从指定的输入目录中，恢复合并后的PNG文件，将其分离为原始的数据文件和图片文件。
    # （Recover files: From the specified input directory, recover the merged PNG file,
    #       and separate it into the original data file and PNG image file.）

    # 一些使用例子（Some usage examples）:

    # 合并文件（Merge files）:
    # python merge.py merge -d /path/to/data_dir -i /path/to/img_dir -o /path/to/output_dir

    # 恢复文件（Recover files）:
    # python merge.py recover -i /path/to/input_dir -o /path/to/output_dir

    # 注意（Note）:

    # 提供的图片必须是 png 格式。合并后的图片文件仍然可以打开。
    # （The provided images must be in PNG format.
    #       The merged PNG image files can still be opened.）

    # 图片数量可以小于数据文件数量。生成的合并文件会以图片文件名命名。
    # （The number of images can be less than the number of data files.
    #       The generated merged files will be named according to the image file names.）

    # 如果图片名字有重复则会自动加计数后缀。
    # （If the image names are repeated, the counting suffix will be automatically added.）
