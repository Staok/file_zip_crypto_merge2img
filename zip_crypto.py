...
if __name__ == "__main__":
    ...
    # 1.Compress (optional using compression password) files or folders: You can choose whether to partition and encrypt the compressed file.

    # 2. （解密并）解压缩文件（或一堆分卷文件），恢复如初。
    # 2. (Decrypt and) decompress files (or a bunch of partitioned files), recover as before.

    # 确保本地 python 环境已经安装了库（Make sure that the local python environment has installed the library）
    # pip install argparse tqdm pyzipper pycryptodome

    # 一些使用例子（Some usage examples）:

    # 获取帮助信（Get help information）:
    # python zip_crypto -h

    # 加密解密的帮助信息（Encrypt and decrypt help information）:
    # python zip_crypto zip_encrypt -h 或（Or） python zip_crypto zip_decrypt -h

    # 压缩并加密给定文件夹，使用分卷 50KB，压缩加密密码为 aaa，在此基础上再加一层文件加密，密码为 abc
    # （Compress and encrypt the given folder, use partitioning 50KB,
    #       compression encryption password is aaa, and add one more file encryption, password is abc）
    # python zip_crypto zip_encrypt -i <input_dir> -o <output_dir> -s 50KB -p aaa -c abc

    # 上面的例子的解压（The above example's decompression）:
    # input_dir 为压缩加密产物的文件夹, 保持这个文件夹除了压缩产物之外没别的以保证分卷文件名称检查不出错
    # （input_dir is the folder where the compressed encryption product is located,
    #       keep this folder without anything else except for the compressed product to ensure that the partitioned file name is checked out correctly）
    # python.exe .\zip_crypto zip_decrypt -i <input_dir> -o <output_dir>  -p aaa -c abc

    # 注意（Note）:
    # 如果加密不使用分卷，则输出为一个文件，则解压缩的时候，指定 -i 为这个文件即可
    # （If encryption is not used for partitioning, the output is a file,
    #       then the decompression can be specified with -i as this file）
